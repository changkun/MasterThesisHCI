...
In training phase, teacher forcing algorithm \cite{williams1989learning} is use, it supplying observed user actions as inputs.
...
a conditional probability under the input clickstream, i.e. we need to maximize
...
\Begin{
    ...
    \For{$\text{clickstream}$ $\in$ $\text{clickstreams}$}{
        \For{page $\in$ $o$)}{
            candidates.append([clickstream.append(page), $p(\text{clickstream}) * p(\text{page})$]) \\
        }
    }
