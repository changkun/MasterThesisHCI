...
\paragraph{Remark 1} This model described by Equation \ref{eqn:url2vecprob} is actually
...
\paragraph{Remark 3} The probability can also be interpretat in Bayesian perspective,
which provides a intuition of this definition. $p(\text{URL}_{t+i} | \text{URL}_t)$
can be considered as a posterial probability. Since $v_{\text{URL}_t}$ was initialized
as a one-hot encoded vector input to the embedding neural network, the item can be treat
...
The original LSTM recurrent unit has a context cell and three regulators: Input gate, output gate and forget gate.
...
which is completely different than the original learnable bias in these gates. 
...
we also use the trick in our model.
